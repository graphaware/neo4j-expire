...
/**
 * A component responsible for handling the indexing of revival dates on nodes and relationships.
 */
public interface RevivalIndexer {

	/**
	 * Add a given node with any relevant revival date property to the expiration index.
	 * If there is no, or a non-numeric revival property, do nothing.
	 *
	 * @param node to index.
	 */
	...
	/**
	 * Finds all indexed nodes that revive before a particular time.
	 *
	 ...
	 */
	IndexHits<Node> candidateNodesRevivingBefore(long timestamp);

	/**
	 * Finds all indexed relationships that revive before a particular time.
	 *
	 * @param timestamp The timestamp to query for, given as milliseconds since epoch.
	 * @return Iterable of all relationships expiring before timestamp.
	 */
	IndexHits<Relationship> candidateRelsRevivingBefore(long timestamp);
